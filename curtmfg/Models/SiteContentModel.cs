...
namespace curtmfg.Models {
    public class SiteContentModel {
        ...
        public static SiteContent GetByID(int id = 0) {
        ...
        public static ContentPage Get(string name = "", int menuid = 0, bool authenticated = false) {
            ContentPage content = new ContentPage();
            try {
                CurtDevDataContext db = new CurtDevDataContext();
                content = (from s in db.SiteContents
                           where s.slug == name && s.published == true && s.active == true
                           select new ContentPage {
                               ...
                               menu = MenuModel.GetByContentID(s.contentID, menuid, authenticated)
                           }).First<ContentPage>();
                return content;
        ...
        public static List<ContentPage> GetSitemap() {
            List<ContentPage> contents = new List<ContentPage>();
            try {
                CurtDevDataContext db = new CurtDevDataContext();
                contents = (from s in db.SiteContents
                            where !(from msc in db.Menu_SiteContents select msc.contentID).Contains(s.contentID)
                           select new ContentPage {
                               contentID = s.contentID,
                               page_title = s.page_title,
                               content_type = s.content_type,
